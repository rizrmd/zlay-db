...
// DuckDB-based CSV driver
pub const CSVDriver = struct {
    const Self = @This();
    ...
    pub fn connect(self: *Self, config: database.ConnectionConfig) errors.DatabaseError!database.Connection {
        ...
        // Create CSV table using DuckDB's CSV reader
        const create_sql_cstr = try std.fmt.allocPrint(self.allocator, "CREATE TABLE csv_data AS SELECT * FROM read_csv_auto('{s}')\x00", .{file_path});
        defer self.allocator.free(create_sql_cstr);

        var result: ?*duckdb_result = null;
        if (duckdb_query(conn.?, create_sql_cstr[0..create_sql_cstr.len :0], &result) != .DUCKDB_STATE_SUCCESS) {
            const error_msg = duckdb_error(result orelse return errors.DatabaseError.ConnectionFailed);
            std.debug.print("DuckDB CSV error: {s}\n", .{error_msg});
        ...
        const conn_data = try self.allocator.create(CSVConnectionData);
        conn_data.* = CSVConnectionData{
            .driver = self,
            .file_path = file_path,
        };
    ...
    pub fn executeQuery(self: *Self, sql: []const u8, args: []const types.Value) errors.DatabaseError!types.ResultSet {
        ...
        // Convert SQL to work with our CSV table
    ...
    pub fn executeUpdate(self: *Self, sql: []const u8, args: []const types.Value) errors.DatabaseError!u64 {
        ...
        // DuckDB doesn't provide affected rows count for CSV operations
        return 0;
    }

    pub fn beginTransaction(self: *Self) errors.DatabaseError!database.Transaction {
        // DuckDB with CSV files doesn't support transactions in traditional sense
        // Return a stub transaction that does nothing
        const transaction_data = try self.allocator.create(CSVTransactionData);
        transaction_data.* = CSVTransactionData{ .driver = self };
    ...
    fn convertSql(self: Self, sql: []const u8) ![*:0]const u8 {
        // Simple SQL conversion - replace table references with csv_data
        ...
        // Replace "FROM <table>" with "FROM csv_data" (basic implementation)
        if (std.mem.indexOf(u8, converted, "FROM")) |pos| {
            ...
            if (start < end) {
                // Replace with csv_data
                const new_sql = try std.fmt.allocPrint(self.allocator, "{s}FROM csv_data{s}\x00", .{ converted[0..after_from], converted[end..] });
...
// CSV connection data
const CSVConnectionData = struct {
    driver: *CSVDriver,
    file_path: []const u8,
};

// CSV transaction data
const CSVTransactionData = struct {
    driver: *CSVDriver,
};

// Create driver interface
fn createDriverInterface(csv_driver: *CSVDriver) database.Driver {
    const driver_data = csv_driver;
...
fn connect_impl(driver_data: *anyopaque, config: database.ConnectionConfig) errors.DatabaseError!database.Connection {
    const csv_driver = @as(*CSVDriver, @ptrCast(@alignCast(driver_data)));
    return csv_driver.connect(config);
}
